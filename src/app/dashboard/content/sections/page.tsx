...
import {
  Lightbulb, Target, Camera, CalendarDots, Plus, TrashSimple,
  CaretDown, CaretUp, PencilSimple, Check, X,
...
const SECTIONS = [
  ...
  { id: 'calendar',      label: 'Content Calendar',icon: CalendarDots, color: '#22c55e', desc: 'Publishing schedule and post timeline' },
] as const;
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Content Strategy Hub</h1>
          <p style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 4 }}>Strategy · Action Plan · Shooting Plan · Calendar</p>
        </div>
        <select
      ...
      {!selectedClient ? (
        ...
        <>
            ...
            return (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-2)' }}>— {sec.desc}</span>
                </div>
          ...
          {/* Items */}
          {loading ? (
            <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-2)', fontSize: 13 }}>Loading...</div>
                  ...
                  {editing === item.id ? (
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={() => setEditing(null)} style={{ padding: '7px 12px', borderRadius: 7, fontSize: 12, cursor: 'pointer', background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-2)' }}>
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        <button onClick={() => startEdit(item)} style={iconBtn}><PencilSimple size={13} /></button>
                        <button onClick={() => deleteItem(item.id)} style={{ ...iconBtn, color: '#ef4444' }}><TrashSimple size={13} /></button>
                      </div>
                    </div>
                  )}
                </div>
...
const iconBtn: React.CSSProperties = {
  width: 28, height: 28, borderRadius: 7, background: 'var(--nav-hover-bg)',
