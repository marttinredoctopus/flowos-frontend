...
import {
  ArrowLeft, TrendUp, CheckCircle, Timer, PaintBrush, CalendarDots,
  Kanban, Lightning, DownloadSimple, ArrowClockwise, Star, ChartBar,
...
export default function ClientReportPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 300 }}>
      <ArrowClockwise size={20} color="#6366f1" style={{ animation: 'spin 1s linear infinite' }} />
    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        <button onClick={() => router.push(`/dashboard/clients/${id}`)}
          style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--nav-hover-bg)',
      ...
      {report.narrative && (
        <div className="fu" style={{
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            <Lightning size={16} color="#6366f1" style={{ flexShrink: 0, marginTop: 2 }} />
            <p style={{ fontSize: 14, color: 'var(--text-3)', lineHeight: 1.7, margin: 0 }}>{report.narrative}</p>
          </div>
        </div>
      )}
      ...
      <div className="fu" style={{ marginBottom: 20,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontSize: 13, color: 'var(--text-2)' }}>Overall Project Completion</span>
        ...
        <div style={{ display: 'flex', gap: 24, marginTop: 12 }}>
          <span style={{ fontSize: 12, color: 'var(--text-2)' }}>
            <Kanban size={11} style={{ display: 'inline', marginRight: 4 }} />
      ...
      <div className="fu" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Tasks Done', value: tasks.completedThisPeriod, sub: `${tasks.pending} pending`, color: '#22c55e', icon: <CheckCircle size={16} /> },
          { label: 'Pending Tasks', value: tasks.pending, sub: `${tasks.total} total`, color: '#f59e0b', icon: <Timer size={16} /> },
          { label: 'Designs Approved', value: designs.approved, sub: `${designs.pendingReview} in review`, color: '#8b5cf6', icon: <PaintBrush size={16} /> },
          { label: 'Content Live', value: content.published, sub: `${content.scheduled} scheduled`, color: '#6366f1', icon: <CalendarDots size={16} /> },
        ].map(s => (
          <div key={s.label} style={{
            ...
            <div style={{ fontSize: 26, fontWeight: 800, color: 'var(--text)', marginBottom: 2 }}>{s.value}</div>
      ...
      <div className="fu" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--nav-hover-bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 20px' }}>
          <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
            <TrendUp size={14} color="#6366f1" /> Recent Activity
          </h3>
          {report.recentActivity.length === 0
            ? <p style={{ fontSize: 13, color: 'var(--text-2)' }}>No activity yet</p>
      ...
      {/* Content breakdown */}
      <div className="fu" style={{ background: 'var(--nav-hover-bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 22px' }}>
        <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
          <CalendarDots size={14} color="#6366f1" /> Content Summary
        </h3>
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          {[
            ...
            { label: 'Drafts', value: content.drafts, color: 'var(--text-2)' },
            { label: 'Total', value: content.total, color: 'var(--text)' },
          ].map(s => (
